...
export default function SpaceCard({
  ...
  return (
    <Card
      ...
      className={`cursor-pointer transition-all duration-300 hover:shadow-lg hover:translate-y-[-4px] flex flex-col h-full ${
        enableJoin ? 'hover:bg-muted/50' : ''
      }`}
    >
      <CardHeader>
      ...
      <CardFooter className="flex justify-between items-center pt-2 mt-auto">
        <Badge
          variant={space.privacy_status ? 'destructive' : 'secondary'}
          className={`px-3 py-1 ${
            !space.privacy_status ? 'bg-green-500 text-white' : ''
          }`}
        >
